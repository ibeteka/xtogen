...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sdx="http://www.culture.gouv.fr/ns/sdx/sdx" xsl:exclude-result-prefixes="sdx">
	...
	<xsl:template match="success">
		...
		<xsl:value-of select="$messages[@id='page.csv.importreussi.debut']"/>&#160;
		<b><xsl:value-of select="rownumber"/></b>&#160;<xsl:value-of select="$messages[@id='page.csv.importreussi.fin']"/>
		<br/><br/>
		<a class="nav" href="admin_edit_liste.xsp?list={list}"><xsl:value-of select="$messages[@id='page.csv.voirlecontenudelaliste']"/></a><br/>
