...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sdx="http://www.culture.gouv.fr/ns/sdx/sdx" exclude-result-prefixes="sdx">
    ...
    <h2><xsl:value-of select="$messages[@id='page.results.resultats']"/></h2>
        <xsl:choose>
            <xsl:when test="@nb > 0">
    ...
    <xsl:template match="sdx:result">
		...
		<xsl:if test="../sdx:result[1]/@no=$no or preceding-sibling::sdx:result[1]/sdx:field[@name='sdxdbid'] != sdx:field[@name='sdxdbid']">
